...
void main() {
    ...
    vec3 perp1 =  normalize(cross(line_dir, dir1)) * size1;
    vec3 perp2 =  normalize(cross(line_dir, dir2)) * size2;

    v_color = color1;
    gl_Position = pv * vec4(p1 - perp1, 1.0);
    EmitVertex();
    ...
    v_color = color2;
    gl_Position = pv * vec4(p2 - perp2, 1.0);
    EmitVertex();
