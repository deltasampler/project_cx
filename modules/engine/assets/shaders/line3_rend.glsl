...
void main() {
    ...
    for (float i = 0; i < (sub + 1); ++i) {
        ...
        v_color = color2;
        gl_Position = pv * vec4(p2 + point * size2, 1.0);
        EmitVertex();
    ...
    // Cap start
    v_color = color2;
    gl_Position = pv * vec4(p2 + perp1 * size2, 1.0);
    EmitVertex();
    ...
    for (int i = 1; i < ceil(sub / 2); ++i) {
        ...
        gl_Position = pv * vec4(p2 + point1 * size2, 1.0);
        EmitVertex();

        gl_Position = pv * vec4(p2 + point2 * size2, 1.0);
        EmitVertex();
    }
